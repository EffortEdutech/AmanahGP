...
export default async function DashboardPage({
  ...
  // orgId comes from URL params — org data loaded from membership join
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        {!onboardingState.isComplete && (
          <Link href={`/org/${orgId}/onboarding`}
      ...
      <OnboardingWidget state={onboardingState} basePath={`/org/${orgId}`} />

      {/* ── KPI cards ── */}
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <KpiCard
          label="Amanah score" href={`/org/${orgId}/trust`}
        ...
        <KpiCard
          label="Certification" href={`/org/${orgId}/certification`}
        ...
        <KpiCard
          label="Active projects" href={`/org/${orgId}/projects`}
          value={String(projectCount)}
          sub="in progress" accent="blue" />
        <KpiCard
          label="Pending approvals" href={`/org/${orgId}/accounting/payment-requests`}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <ModuleCard href={`/org/${orgId}/accounting`}           icon="$"  label="Accounting"     description="Fund accounting, transactions, financial statements" />
          <ModuleCard href={`/org/${orgId}/trust`}                icon="▲"  label="Trust score"    description="Amanah Index breakdown and improvement tips" />
          <ModuleCard href={`/org/${orgId}/accounting/payment-requests`} icon="✉" label="Payment requests" description="Approval workflow — segregation of duties" />
          <ModuleCard href={`/org/${orgId}/projects`}             icon="▦"  label="Projects"       description="Create and manage charitable projects" />
          <ModuleCard href={`/org/${orgId}/compliance`}           icon="☑"  label="Compliance"     description="ROS, MAIN, and regulatory reports" />
          <ModuleCard href={`/org/${orgId}/policy-kit`}           icon="⊞"  label="Policy kit"     description="Governance templates and Zakat SOP" />
          <ModuleCard href={`/org/${orgId}/onboarding`}           icon="◉"  label="Amanah Ready"   description={`Setup progress — ${onboardingState.pct}% complete`} />
          <ModuleCard href={`/org/${orgId}/certification`}        icon="★"  label="Certification"  description="CTCF certification application" />
          <ModuleCard href={`/org/${orgId}/members`}              icon="♟"  label="Team"           description="Members, roles, and invitations" />
        </div>
      </div>
