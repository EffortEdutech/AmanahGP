...
export default async function ReportsPage({
  ...
  if (sp.filter === 'pending') query = query.eq('verification_status', 'pending').eq('submission_status', 'submitted');
  if (sp.filter === 'verified') query = query.eq('verification_status', 'verified');
  if (sp.filter === 'draft') query = query.eq('submission_status', 'draft');
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-5">
      ...
      <div className="flex items-start justify-between">
        ...
        {isManager && (
          <Link href={`/org/${orgId}/reports/new`}
      ...
      {reports && reports.length > 0 ? (
        <div className="rounded-lg border border-gray-200 bg-white divide-y divide-gray-100">
          {reports.map((report) => {
            ...
            return (
              <Link key={report.id} href={`/org/${orgId}/reports/${report.id}`}
