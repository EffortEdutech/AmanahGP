...
export default async function TransactionsPage({
  ...
  const filteredEntries = sp.fundId
    ? (entries ?? []).filter((e) =>
        relationMany<{ fund_id: string }>(e.journal_lines).some((l) => l.fund_id === sp.fundId)
      )
    : (entries ?? []);
  ...
  const fundExtraParam = sp.fundId ? `&fundId=${sp.fundId}` : '';
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-5">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <MonthYearPicker
            ...
            basePath={`/org/${orgId}/accounting/transactions`}
            extraParams={fundExtraParam}
            allowAllMonths
          />
          {isManager && (
            <Link href={`/org/${orgId}/accounting/transactions/new`}
      ...
      {funds && funds.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          <Link
            href={`/org/${orgId}/accounting/transactions?year=${selectedYear}${selectedMonth ? `&month=${selectedMonth}` : ''}`}
          ...
          {funds.map((f) => (
            <Link key={f.id}
              href={`/org/${orgId}/accounting/transactions?year=${selectedYear}${selectedMonth ? `&month=${selectedMonth}` : ''}&fundId=${f.id}`}
              className={`px-3 py-1 text-[11px] font-medium rounded-full border transition-colors ${
                sp.fundId === f.id
