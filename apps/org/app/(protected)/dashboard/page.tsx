...
// amanahOS — Dashboard
//
// Uses service client for org_members query — same RLS bypass reason as layout.tsx.
// auth.uid() ≠ org_members.user_id due to seed UUID mismatch on hosted Supabase.
...
export default async function DashboardPage() {
  ...
  // Resolve platformUser via auth_provider_user_id
  ...
  // Load first org membership via service client (bypasses RLS UUID mismatch)
  const { data: membership } = await service
    ...
    .select(`
      ...
      org_role,
      organizations (
        id, name, onboarding_status, listing_status, org_type, state
      )
    `)
  ...
  if (!membership) {
    return (
      <div className="p-8 text-center text-gray-500 space-y-2">
        <p className="font-medium text-gray-700">No organisation found for your account.</p>
  ...
  // All subsequent queries use regular client — these tables have public/org-scoped RLS
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      {!onboardingComplete && (
        <div className="rounded-lg bg-amber-50 border border-amber-200 p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-amber-800">Organisation setup in progress</p>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <KpiCard
          ...
          sub="out of 100"
          accent={amanahScore === null ? 'gray' : amanahScore >= 70 ? 'green' : amanahScore >= 50 ? 'amber' : 'red'}
        />
        <KpiCard
          label="Certification" href="/compliance"
        ...
        />
        <KpiCard
          label="Reports pending" href="/reports"
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <ModuleCard href="/profile"    icon="◎" label="Profile"    description="Organisation details, classification, contact info" />
          <ModuleCard href="/projects"   icon="▦" label="Projects"   description="Create and manage your charitable projects" />
          <ModuleCard href="/reports"    icon="✎" label="Reports"    description="Submit progress reports and upload evidence" />
          <ModuleCard href="/accounting" icon="$" label="Accounting" description="Fund accounting, transactions, financial statements" badge="Phase 2" />
          <ModuleCard href="/compliance" icon="☑" label="Compliance" description="ROS, MAIN, and donor transparency reports" badge="Phase 2" />
          <ModuleCard href="/governance" icon="⊞" label="Policy kit" description="Governance templates, Zakat SOP, PDPA policy" badge="Phase 2" />
...
function ModuleCard({ href, icon, label, description, badge }: {
  href: string; icon: string; label: string; description: string; badge?: string;
}) {
  return (
    <Link href={href} className="rounded-lg border border-gray-200 bg-white p-4 hover:border-emerald-300 hover:shadow-sm transition-all flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <span className="text-lg text-gray-600">{icon}</span>
        {badge && <span className="text-[8px] font-medium px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700">{badge}</span>}
      </div>
      <div>
